...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    protected void upHeap(int index) {
    	while (index > 0) {
    		...
    		if (compare(list.get(index).getKey(), list.get(parentIndex).getKey()) >= 0) {
    			break;
    		} else {
    ...
    protected void downHeap(int index) {
    	while (hasLeft(index)) {
            ...
            if (compare(list.get(smallerChildIndex).getKey(), list.get(index).getKey()) > 0) {
            	break;
            } else {
