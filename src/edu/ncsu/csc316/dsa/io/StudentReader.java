...
public class StudentReader {
	...
	private static Student processLine(String line) {
		...
		String first = studentParts[0];
		String last = studentParts[1];
		String unityId = studentParts[2];
		int studnetId = Integer.parseInt(studentParts[3]);
		double gpa = Double.parseDouble(studentParts[4]);
		int creditHours = Integer.parseInt(studentParts[5]);
		return new Student(first, last, studnetId, creditHours, gpa, unityId);
	}
}
