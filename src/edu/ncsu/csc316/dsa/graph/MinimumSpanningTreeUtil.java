...
public class MinimumSpanningTreeUtil {
    ...
    public static <V, E extends Weighted> PositionalList<Edge<E>> kruskal(Graph<V, E> g) {
    	...
    	AdaptablePriorityQueue<Integer, Edge<E>> pEdges = new HeapAdaptablePriorityQueue<Integer, Edge<E>>();
    	DisjointSetForest<Vertex<V>> forest = new UpTreeDisjointSetForest<Vertex<V>>();
    	
    	for (Edge<E> e : g.edges()) {
    		pEdges.insert(e.getElement().getWeight(), e);
    	}
    	...
    	while (components > 1) {
    		Entry<Integer, Edge<E>> entry = pEdges.deleteMin();
    		Edge<E> edge = entry.getValue();
    		Vertex<V>[] endpoints = g.endVertices(edge);
    		Position<Vertex<V>> endPoint1 = forest.find(endpoints[0]);
    		Position<Vertex<V>> endPoint2 = forest.find(endpoints[1]);
    		if (endPoint1 != endPoint2) {
    			forest.union(endPoint1, endPoint2);
    			edges.addLast(edge);
    			components -= 1;
    		}
