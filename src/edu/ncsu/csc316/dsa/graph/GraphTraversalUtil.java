...
public class GraphTraversalUtil {
    ...
    public static <V, E> Map<Vertex<V>, Edge<E>> depthFirstSearch(Graph<V, E> graph, Vertex<V> start) {
    	Set<Vertex<V>> visetedVertex = new HashSet<Vertex<V>>();
    	Map<Vertex<V>, Edge<E>> dEdge = new LinearProbingHashMap<Vertex<V>, Edge<E>>();
    	dfsHelper(graph, start, visetedVertex, dEdge);
    	return dEdge; 
    }
    ...
    public static <V, E> Map<Vertex<V>, Edge<E>> breadthFirstSearch(Graph<V, E> graph, Vertex<V> start) {
    	Set<Vertex<V>> vVertex = new HashSet<Vertex<V>>();
    	Map<Vertex<V>, Edge<E>> dEdge = new LinearProbingHashMap<Vertex<V>, Edge<E>>();
    	Queue<Vertex<V>> rVertex = new ArrayBasedQueue<Vertex<V>>();
    	
    	vVertex.add(start);
    	rVertex.enqueue(start);
    	while (!rVertex.isEmpty()) {
    		Vertex<V> u = rVertex.dequeue();
    		for (Edge<E> e : graph.outgoingEdges(u)) {
    			Vertex<V> w = graph.opposite(u, e);
    			if (!vVertex.contains(w)) {
    				vVertex.add(w);
    				dEdge.put(w, e);
    				rVertex.enqueue(w);
    			}
