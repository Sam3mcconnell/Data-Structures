...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
        ...
        v1.getOutgoing().addLast(edge);
        edge.setOutgoingListPosition(v1.getOutgoing().last());

        if (!isDirected()) {
            v2.getIncoming().addLast(edge);
            edge.setIncomingListPosition(v2.getIncoming().last());
        }
