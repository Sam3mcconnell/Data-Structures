...
public class AdjacencyMapGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> v1, Vertex<V> v2, E edgeData) {
        ...
        Edge<E> e = super.insertEdge(origin, destination, edgeData);
    ...
    public void removeEdge(Edge<E> edge) {
        ...
        AMVertex v1 = validate(ends[0]);
        AMVertex v2 = validate(ends[1]);

        v1.getOutgoing().remove(ends[1]);
        
        if (!isDirected()) {
            v2.getIncoming().remove(ends[0]);
        }
